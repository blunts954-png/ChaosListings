...
export default function EditBusinessForm({ businessId }: { businessId: string }) {
  ...
  useEffect(() => {
    if (user && user.agencyId) {
      api.get(`/agencies/${user.agencyId}/businesses/${businessId}`)
        .then(response => {
          ...
          setGmbUrl(biz.gmbUrl);
          setReviewUrl(biz.reviewUrl);
        })
        .catch(err => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user || !user.agencyId) {
        setError("You must be part of an agency to edit a business.");
        return;
    }

    try {
      await api.patch(`/agencies/${user.agencyId}/businesses/${businessId}`, { name, address, phone, website, gmbUrl, reviewUrl });
  ...
  if (error && !business) return <ErrorMessage message={error} />;
